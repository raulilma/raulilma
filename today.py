...
USER_NAME = os.environ['USER_NAME'] # 'raulilma'
...
def make_request_with_retries(url, json_data, headers, max_retries=3, delay=5):
    for attempt in range(max_retries):
        try:
            response = requests.post(url, json=json_data, headers=headers, timeout=(10, 30))
            response.raise_for_status()
            return response
        except (requests.exceptions.ChunkedEncodingError, requests.exceptions.RequestException) as e:
            print(f"Attempt {attempt + 1} failed: {e}")
            time.sleep(delay)
    raise Exception("Max retries reached, failed to get a valid response.")
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = make_request_with_retries('https://api.github.com/graphql', json={'query': query, 'variables': variables}, headers=HEADERS)
    if request.status_code == 200:
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    ...
    if request.status_code == 200:
        if request.json()['data']['user']['repositories']['pageInfo']['hasNextPage']:
            print(request.json()['data']['user']['repositories'])
...
def user_getter(username):
    ...
    raise Exception('user_getter() has failed with a', variables, request.status_code, request.text, QUERY_COUNT)
...
def formatter(query_type, difference, funct_return=False, whitespace=0):
    ...
    if whitespace:
        return '{:,}'.format(funct_return)
    return funct_return


if __name__ == '__main__':
    """
    Raul Ilma Rajasa (raulilma), 2022
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'MDQ6VXNlcjQwNDIwODU0'} and 2018-06-20T07:04:33Z for username 'raulilma'
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    print(user_getter(USER_NAME))
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2002, 9, 29))
    ...
    contrib_data, contrib_time = perf_counter(graph_repos_stars, 'repos', ['OWNER'])

    # several repositories that I've contributed to have since been deleted.
    if OWNER_ID == {'id': 'MDQ6VXNlcjQwNDIwODU0'}: # only calculate for user raulilma
